...
- **Schema**:
    ...
    - `transactions`: Point credit/debit history. Includes optional `category_id` FK for tagging transactions.
    - `transaction_categories`: Org-scoped categories for tagging reward transactions (name, color).
    - `monthly_reports`: Stored monthly JSONB reports per org (year, month, reportData JSON).
...
- **Transaction Categories + Analytics**: Prime admins define color-coded categories (e.g. "Safety", "Performance") in Settings → Category Management. Admins select a category when issuing credit transactions in Instant Transaction. Dashboard analytics show per-category totals with budget progress bars. API routes: `GET/POST /api/admin/categories`, `PATCH/DELETE /api/admin/categories/:id`, `GET /api/admin/analytics/categories`.
- **Monthly Reports + Documents Page**: End-of-month cron (1st of month, 2 AM UTC) auto-generates a JSONB summary report per org (total credited, debited, top employees, category breakdown, active employees). Admin nav → Documents (`/admin/documents`): lists all reports, lets prime admins generate on demand (`POST /api/admin/reports/generate`), and renders a print-friendly preview via `window.print()`. API routes: `GET /api/admin/reports`, `POST /api/admin/reports/generate`, `GET /api/admin/reports/:id`.
